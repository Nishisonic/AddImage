...
const KC_ASSETS = ["banner", "banner_dmg"];
...
writeFile("ci/START2.json", JSON.stringify(masterData), (err) => {
  if (err) {
...
Promise.all(
  KC_ASSETS.map((assets) => {
    if (!existsSync(`Image/Ship/${assets}`)) {
      mkdirSync(`Image/Ship/${assets}`);
    }
    return ships
      .filter(({ api_getmes }) => api_getmes)
      .map((ship) => ship.api_id)
      .map((id) => `Image/Ship/${assets}/${id}.png`)
      .filter((file) => {
        try {
          accessSync(file);
          return false;
